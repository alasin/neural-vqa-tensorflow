...
def load_questions_answers(opts):
    ...
    t_q_json_file = join(
        opts.data_dir, 'MultipleChoice_mscoco_train2014_questions.json')
    t_a_json_file = join(opts.data_dir, 'mscoco_train2014_annotations.json')

    v_q_json_file = join(
        opts.data_dir, 'MultipleChoice_mscoco_val2014_questions.json')
    ...
    question_vocab, max_question_length = make_questions_vocab(
        questions, answers, answer_vocab)
...
def make_answer_vocab(answers):
    ...
    answer_frequency_tuples = [(-frequency, answer)
                               for answer, frequency in answer_frequency.iteritems()]
...
def make_questions_vocab(questions, answers, answer_vocab):
    ...
    qw_freq_threhold = 0
    qw_tuples = [(-frequency, qw)
                 for qw, frequency in question_frequency.iteritems()]
    # qw_tuples.sort()
